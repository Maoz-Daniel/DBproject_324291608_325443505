...
- [שלב ה: ממשק גרפי](#שלב-ה-ממשק-גרפי)
  - [תיאור ארכיטקטורת הממשק והכלים בהם השתמשנו](#תיאור-ארכיטקטורת-הממשק-והכלים-בהם-השתמשנו)
  - [מסכים](#מסכים)
...
## שלב ה: ממשק גרפי
הממשק הגרפי של המערכת פותח באמצעות React ו־TypeScript, בשביל לייצר ממשק ידידותי ומקצועי
הממשק כולל חמישה מסכים עיקריים:
Dashboard, ניהול מנויים, ניהול אזורים, ניהול רשומות כניסה ויציאה, ומסך דוחות.
במסך הדוחות ניתן להריץ שאילתות, פרוצדורות ופונקציות שונות, לקבלת ניתוחים וסטטיסטיקות מתוך בסיס הנתונים.

###  תיאור ארכיטקטורת הממשק והכלים בהם השתמשנו
המערכת בנויה בארכיטקטורת **Frontend–Backend–API**, שמפרידה בין ממשק המשתמש, הלוגיקה העסקית וקריאות הנתונים. מבנה זה מאפשר תחזוקה נוחה והפרדת אחראויות 

#### Frontend
הפרונטאנד פותח באמצעות **React** עם **TypeScript**, בשיטת רכיבים (Components) מופרדים לפי תחומים - ניהול מנויים, אזורים, יומני כניסה/יציאה, דוחות ועוד.  
בנוסף, השתמשנו ב־**TailwindCSS** לעיצוב מודרני ויעיל, וב־**React Hooks** (כמו `useApi`) לניהול קריאות API בצורה מרוכזת ונקייה.
#### Backend
הבאקאנד נכתב ב־**FastAPI**, פריימוורק מהיר ונוח ב־Python. הוא מחולק למודולים:
- `routes` - עבור הגדרת נקודות הקצה (endpoints) לפי תחום  
- `models` - לתיאור הישויות במסד הנתונים  
- `database` - לניהול החיבור למסד  
- `sql_runner` - להרצת שאילתות SQL מותאמות
...
###  מסכים
#### Dashboard
מסך הדאשבורד משמש כעמוד הבית של האפליקציה, ודרכו ניתן לנווט לשאר חלקי המערכת: ניהול מנויים, ניהול אזורים, רשומות כניסה ויציאה, ודוחות.
- ![dashboard](Stage_E/images/dashboard.png)

#### Members
מסך זה מאפשר לנהל את רשימת המנויים הרשומים למערכת. ניתן לצפות בפרטי המנויים, להוסיף מנויים חדשים, לערוך פרטים קיימים ולבצע מחיקה של מנויים.
##### התממשקות ל־ API
כל פעולה במסך (צפייה, הוספה, עריכה, מחיקה) מתבצעת באמצעות קריאות API לשרת. הקריאות נשלחות דרך ה־hook `useApi.ts`, ומתחברות לנקודות קצה שנכתבו ב־FastAPI, כמו למשל:
- `GET /members` - לקבלת רשימת מנויים
- `POST /members` - להוספת מנוי חדש
- `PUT /members/{id}` - לעדכון פרטי מנוי
- `DELETE /members/{id}` - למחיקת מנוי
...
#### Reports  
מסך זה מאפשר להריץ דוחות מתקדמים מהמערכת: שאילתות (queries), פונקציות ופרוצדורות. המשתמש יכול לבחור דוח מסוים, להזין קלטים נדרשים במידת הצורך, ולקבל את התוצאה בטבלה ברורה. הדוחות נוגעים להיבטים כמו תנועת מתאמנים, נתוני עובדים, תיקונים, זמני שהייה, ועוד.

##### התממשקות ל־ API  
כל שאילתה, פונקציה או פרוצדורה מבוצעת דרך קריאות API המוגדרות מראש בקובץ `ReportsView.tsx`, ומתבצעות דרך `useApi.ts` אל נקודות הקצה שנכתבו ב־FastAPI. בהתאם לסוג הדוח, הקריאה מתבצעת בשיטת `GET` או `POST`, ויכולה לכלול פרמטרים כמו מספר חודשים או עלות שירות.

##### חיבור לבסיס הנתונים  
בצד השרת, כל קריאה עוברת דרך פונקציות Python שמריצות את השאילתה או הפרוצדורה הרצויה מול בסיס הנתונים PostgreSQL. כל דוח ממופה לפונקציה ב־backend אשר מריצה שאילתה ישירה או מפעילה פרוצדורה מוגדרת מראש. הנתונים נשלפים מטבלאות כמו `entryRecord`, `gym`, `zone`, `repair`, `member`, `person`, `exitRecord`, ועוד.
...
#### התקנה והפעלה  
כאן נמצאות הוראות להתקנת כל התלויות, הפעלת בסיס הנתונים, והרצת ה־Backend וה־Frontend של המערכת.
...
##### הפעלת בסיס הנתונים  
המערכת עושה שימוש במסד נתונים מסוג **PostgreSQL**. ניתן להריץ את מסד הנתונים באופן מקומי או באמצעות Docker.

אם אתם משתמשים ב־Docker, ודאו שהקונטיינר רץ עם הגדרות תואמות (שם משתמש, סיסמה, שם מסד) כפי שמוגדרים בקובץ `.env`.

---

##### הרצת ה Bsckend 
לאחר התקנת התלויות, ניתן להריץ את שרת ה־FastAPI בעזרת הפקודה הבאה:

```bash
uvicorn gym_backend.api.main:app --reload
```
שרת זה יאזין לקריאות API המגיעות מה־Frontend, ויתחבר למסד הנתונים PostgreSQL.

##### הרצת ה Frontend
יש להיכנס לתיקיית project ולהריץ את הפקודה:

```bash
npm run dev
```
שרת הפיתוח של Vite יפתח את הממשק הגרפי בדפדפן (ברירת מחדל: http://localhost:5173), ויתחיל להאזין לשינויים בקוד.

...
**תיאור שגיאה:**  
לאחר הוספת האילוצים, ניסינו להזין ערכים לא חוקיים – כגון `membershipType` שלא קיים ברשימת הסוגים המותרים (למשל: "Weekly"), או `entryTime` שממוקם בעתיד (כמו: 2099-01-01 00:00:00).  
המערכת חסמה את ההכנסות וזרקה שגיאת `CHECK CONSTRAINT` כמצופה.

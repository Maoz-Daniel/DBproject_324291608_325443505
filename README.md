...
### תיאור ארכיטקטורת הממשק
המערכת בנויה בארכיטקטורת Frontend–Backend–API.
צד הלקוח (Frontend) פותח ב־React + TypeScript ומחולק לרכיבים לפי תחומים (Dashboard, Members, Logs, Reports ועוד), עם שימוש ב־hooks ו־types מותאמים.
צד השרת (Backend) ממומש ב־FastAPI ומארגן את הקוד לפי מודולים (routes, models, database), כאשר התקשורת בין הפרונט לאחור נעשית באמצעות קריאות API מאובטחות ומופרדות לפי תחום.